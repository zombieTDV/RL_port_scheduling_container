...
func _compute_reward(action: Array) -> float:
	...
	# 5. Success bonus on arrival: requires facing box within ~70 deg and coming to controlled stop <= 0.30 m/s
	if cur_dist <= grasp_reach_threshold:
		if alignment >= 0.35:
			if amr.current_speed <= stop_speed_threshold:
				...
				# Near box but still cruising: encourage linear braking
