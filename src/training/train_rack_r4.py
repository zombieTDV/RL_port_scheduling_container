#!/usr/bin/env python3
"""Stage R4: Full Rack Cycle (Rack Pick & Conveyor Dropoff) PPO Training.

Warm-starts directly from Stage R3 (16-D PPO checkpoint) to learn the complete
autonomous warehouse loop:
  1. Multi-tier rack approach and precision IK docking
  2. Active trigger extraction and dynamic tray stowing
  3. Bay corridor transit while carrying payload
  4. Elevated conveyor table docking and alignment
  5. Dynamic unstow and placement flush onto conveyor deck
"""
...
class RackCycleSuccessCallback(BaseCallback):
    """Monitors rack pick, tray stowing, conveyor placement, and cycle success rate."""

    def __init__(self, verbose: int = 1) -> None:
        super().__init__(verbose)
        self.pick_successes: List[bool] = []
        self.stow_successes: List[bool] = []
        self.place_successes: List[bool] = []
        self.cycle_successes: List[bool] = []
    ...
    def _on_step(self) -> bool:
        ...
        for idx, done in enumerate(dones):
            if done and idx < len(infos):
                ...
                cycle = bool(info.get("cycle_success", False))
                toppled = bool(info.get("rack_toppled", False))

                self.pick_successes.append(picked)
                self.stow_successes.append(stowed)
                self.place_successes.append(placed)
                self.cycle_successes.append(cycle)
                self.topple_events.append(toppled)

                # Keep sliding window
                if len(self.cycle_successes) > self.recent_window:
                    self.pick_successes.pop(0)
                    self.stow_successes.pop(0)
                    self.place_successes.pop(0)
                    self.cycle_successes.pop(0)
                    self.topple_events.pop(0)

                if len(self.cycle_successes) >= 10 and len(self.cycle_successes) % 10 == 0:
                    pick_rate = float(np.mean(self.pick_successes))
                    stow_rate = float(np.mean(self.stow_successes))
                    place_rate = float(np.mean(self.place_successes))
                    cycle_rate = float(np.mean(self.cycle_successes))
                    topple_rate = float(np.mean(self.topple_events))
                    print(
                        f"  [Rack R4] Steps: {self.num_timesteps:6d} | "
                        f"Pick: {pick_rate * 100:5.1f}% | "
                        f"Stow: {stow_rate * 100:5.1f}% | "
                        f"Cycle Success: {cycle_rate * 100:5.1f}% | "
...
def main() -> None:
    ...
    def_base_port = int(train_cfg.get("base_port", 11110)) + 40
    ...
    parser = argparse.ArgumentParser(description="Train Stage R4 Full Rack Cycle PPO")
    ...
    parser.add_argument("--warm-start", type=str, default=def_warm_start, help="Source checkpoint (default: ppo_r3_final.zip)")
    ...
    print("=" * 72)
    print("   STAGE R4: FULL RACK CYCLE PPO TRAINING")
    ...
    success_callback = RackCycleSuccessCallback()
    checkpoint_callback = CheckpointCallback(
        save_freq=max(2048, args.steps // 10),
        save_path=os.path.join(args.model_dir, "r4"),
        name_prefix="ppo_r4",
    )
    ...
    if not args.no_warm_start and os.path.isfile(args.warm_start):
        print(f">> Warm-starting R4 policy from: {args.warm_start}")
    ...
    try:
        ...
        final_path = os.path.join(args.model_dir, "ppo_r4_final.zip")
        model.save(final_path)
        print(f"\n✔ Stage R4 training complete! Model saved to: {final_path}")

        # Export native in-engine JSON policy for zero-latency execution
        json_path = "godot/models/ppo_r4_policy.json"
